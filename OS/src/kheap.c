...
{
    size_t size = heap->end_address - heap->start_address;
    printk("Heap start: 0x%x\nHeap End: 0x%x\nHeap Size: 0x%x (%d Bytes)\n",
        heap->start_address, heap->end_address, size, size);
    log(LOG_INFO, "Heap start: 0x%x\nHeap End: 0x%x\nHeap Size: 0x%x (%d Bytes)\n",
        heap->start_address, heap->end_address, size, size);
}
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    // ...And we're done!
    printk("Successfully allocated %d bytes\n", size);
    log(LOG_INFO, "Successfully allocated %d bytes\n", size);
